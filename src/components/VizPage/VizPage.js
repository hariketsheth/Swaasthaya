...
const VizPage = () => {
  ...
  const tooltipRender = ({ point }) => {
    const { value } = point;
    return <span>Longest Streak: {value.current}</span>;
  };

  const tooltipRenderCurrent = ({ point }) => {
    const { value } = point;
    return <span>Current Streak: {value.current}</span>;
  };
