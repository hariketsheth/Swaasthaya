...
const OverlayCard = ({ user, habit, toggleExpand, allLabels, stats }) => {
  ...
  const updateHabitDone = () => {
    ...
    var new_xp = (xp + 25) % 100;
    if (new_xp < xp) {
      currLevel++;
    }
    db.collection("stats").doc(user.uid).update({
      currLevel: currLevel,
      xp: new_xp,
    });
  };
  ...
  return (
    ...
    >
      <div
        onDoubleClick={() => toggleExpand(false)}
        className="overlay__card-textarea"
      >
        <Form
          ...
          render={(formRenderProps) => (
            <FormElement style={{}}>
              <Field
                ...
                onChange={() =>
                  setDescription(formRenderProps.valueGetter("habitTextarea"))
                }
              />
            </FormElement>
          )}
      ...
      <div className="habit__buttons">
        <Button className="save-habbit__button" onClick={updateHabitDetails}>
          Save Changes
        </Button>
      </div>
