...
const GardenMini = () => {
        ...
        db.collection("gardens")
          ...
          .onSnapshot((snapshot) => {
            ...
            temp_cloud.forEach((coordinate) => {
              ...
              hasItem[temp__coordinates[0] * 10 + temp__coordinates[1]] = [
                coordinate[0],
                true,
              ];
            });
  ...
  useEffect(() => {
    ...
    for (let x = 0; x < 10; x++) {
      for (let y = 0; y < 10; y++) {
        const piece = hasItem[10 * x + y][1] ? (
          <GardenItem gardenItemName={hasItem[10 * x + y][0]} />
        ) : null;
        tempSquares.push(
          <div
            onClick={(e) => handleSquareClick(e, hasItem[10 * x + y][1], x, y)}
            key={10 * x + y}
  ...
  const updateMiniGarden = () => {
    ...
    hasItem.forEach(function (item, idx) {
      if (item[0] !== "empty") {
        true_points.push({
          idx: [item[0], 9 - (idx % 10), Math.floor(idx / 10)],
        });
      }
    });
  ...
  const handleSquareClick = function (event, containsItem, x, y) {
    ...
    if (containsItem) {
      ...
      if (square_ref.current != null) {
        ...
        var last_x = lastCoord[0],
          last_y = lastCoord[1];
        var temp_has_state = hasItem;
        temp_has_state[10 * x + y] = [
          temp_has_state[10 * last_x + last_y][0],
          true,
        ];
