...
const DailyHabits = () => {
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged((authUser) => {
      if (authUser) {
        ...
        db.collection("allLabels").onSnapshot((snapshot) => {
          setAllLabels([...snapshot.docs.map((doc) => doc.data().labels)]);
        });
  ...
  const addHabit = (e) => {
    ...
    for (let i = 0; i < daysCumSum.length - 1; i++) {
      var curr_slice = hh_copy.slice(
        hh_copy.length - daysCumSum[i],
        hh_copy.length - daysCumSum[i + 1]
      );
  ...
  function SkeletonOrHabits(props) {
    ...
    if (!habits) {
      ...
      return habits.map((habit) => (
        <div key={habit.id} className="daily__habit-wrapper">
          <div
            onClick={(e) => {
          ...
          >
            <HabitCard
              ...
              title={habit.habit}
              bgc={
                habit.habitHistory[habit.habitHistory.length - 1]
                  ? "#00ff00"
                  : "#ffffff"
              }
              lab={habit.habitLabels}
            />
          </div>
          <motion.div
      ...
      <div className="habits_container">
        ...
        <ol className="habit__list">
          <div className="daily__habits-input">
            ...
            <Button disabled={!input.length} onClick={addHabit}>
              Add Habit
            </Button>
          </div>
        ...
        {wantDelete && (
          ...
          >
            <div
              ...
              className="delete__habit-buttons"
            >
              <Button
                className="delete__habit-del"
                onClick={() => deleteHabit(currentHabit)}
              >
                Yes, Delete
              </Button>
              <Button
                className="delete__habit-cancel"
        ...
        {premium && (
          <Dialog
            title={"Want more habits? Upgrade to premium 🤩!"}
          ...
          >
            ...
            >
              <Button
                style={{ backgroundColor: "#36E46B", borderRadius: "10px" }}
                onClick={() => setPremium(false)}
              >
                Upgrade to Kendo-Pro!
              </Button>
            </div>
          </Dialog>
        )}
