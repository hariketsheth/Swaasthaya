...
const RegisterModal = () => {
    ...
    if (name && email && password) {
      signup(email, password)
        .then((userCredential) => {
          ...
          db.collection("gardens").doc(user.uid).set({
            coordinates: true_points,
          });

          db.collection("stats").doc(user.uid).set({
            currLevel: 1,
            xp: 0,
          });
  ...
  const togglePassword = () => {
    ...
    const type =
      togglePass.getAttribute("type") === "password" ? "name" : "password";
  ...
  return (
    <Card className="register__modal">
      <CardHeader className="register__modal-header">Register</CardHeader>
      <Form
        onSubmit={() => {}}
          ...
          <FormElement style={{ width: 400 }}>
            ...
            >
              ...
              >
                <FontAwesomeIcon
                  size="3x"
                  icon={eye === "faEye" ? faEye : faEyeSlash}
                />
              </motion.div>
            </div>
